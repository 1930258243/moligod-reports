# -*- coding: utf-8 -*-
"""cloud_report_all.py — 全子弹版报告（68 款：26 三级 + 26 四级 + 16 五级）
在 cloud_report.py 基础上扩展：5 级弹仅实时行情（无 30 天 K 线，只进行情表/榜单/独立板块）。
口径：低价期=8/25-9/2；日常期=8/2-8/24；出售到手=挂价*0.87；SPIKE 不特殊处理。
用法: python cloud_report_all.py [daily|weekly|monthly] [output.html]
"""
...
LOW_START, LOW_END = '2026-08-25', '2026-09-02'
DAY_START, DAY_END = '2026-08-02', '2026-08-24'
TAX = 0.87
...
def build_rows_all(history, catalog):
    """全子弹：3/4/5 级都进 rows。5 级只有实时行情（无 low_*/day_*/profit）。"""
    rows = []
    for iid, cat in catalog.items():
        h = history.get(iid)
        grade = str(cat.get('grade'))
        latest = cat.get('latest')
        th, tl = cat.get('today_high'), cat.get('today_low')
        yh, yl = cat.get('yesterday_high'), cat.get('yesterday_low')
        ...
        row = {
            'name': cat.get('name') or iid, 'grade': grade, 'latest': latest,
            'th': th, 'tl': tl, 'prev': prev, 'chg': chg, 'amp': amp,
            'seven_high': cat.get('seven_high'), 'seven_low': cat.get('seven_low'),
            'low_low': None, 'low_avg': None, 'low_high': None,
            'day_low': None, 'day_avg': None, 'day_high': None,
            'tax_net': None, 'profit': None, 'margin': None,
            'cur_space': None, 'cur_margin': None,
        }
        if h:
...
def render_overview(rows, today):
    with_chg = [r for r in rows if r['chg'] is not None]
    idx = {}
    for g in ('3', '4', '5'):
        grp = [r for r in with_chg if str(r['grade']) == g]
        idx[g] = (sum(r['chg'] for r in grp) / len(grp)) if grp else None
    low_tag = '低价期最后一天' if today >= LOW_END else ('低价期进行中' if today >= LOW_START else '日常期')
    tags = []
    for g, label in (('3', '3级'), ('4', '4级'), ('5', '5级')):
        if idx[g] is not None:
            tags.append('<span class="tag">%s指数 %s</span>' % (label, pct_str(idx[g])))
        else:
            tags.append('<span class="tag">%s指数 --</span>' % label)
    tags.append('<span class="tag">覆盖 %d 种子弹（%d 三级 / %d 四级 / %d 五级）</span>' % (
        len(rows),
        sum(1 for r in rows if str(r['grade']) == '3'),
        sum(1 for r in rows if str(r['grade']) == '4'),
        sum(1 for r in rows if str(r['grade']) == '5')))
...
def render_daily(rows, today):
    ...
    return """<h2>今日行情表（全部 {n} 种，按最新价涨跌幅）</h2>
...
<div class="warn"><b>风险提示：</b> {risk}</div>""".format(
        ...
        amp_tbody=amp_tbody,
        v=len(valid), full_tbody=''.join(full_tbody), cur_tbody=''.join(cur_tbody),
        nl='<br>'.join(nl), ol=ol, risk=risk)

def render_weekly_monthly(rows, rtype, today):
    t = {'weekly': '行情周报', 'monthly': '行情月报'}.get(rtype, '行情报告')
    body = ''
    valid = [r for r in rows if r['margin'] is not None]
    valid.sort(key=lambda r: -(r['margin'] or -999))
    ft = []
    for r in valid:
        ft.append('<tr><td>%s</td><td>%s</td><td>%s</td><td>%s / %s / %s</td><td>%s / %s / %s</td>'
            ...
            r['tax_net'], cls(r['margin']), r['profit'], r['margin']))
    pos_n = sum(1 for r in valid if (r['margin'] or 0) > 0)
    body += '<h2>%s 全表：低价期 vs 日常期（全部 %d 种，按税后利润率排序）</h2>' % (t, len(valid))
    body += '<div class="sub">利润 = 日常期均价×0.87 − 低价期均价；低价期可买(税后正利润) <b class="pos">%d</b> 款</div>' % pos_n
...
def main():
    ...
    out = sys.argv[2] if len(sys.argv) > 2 else os.path.join(BASE, 'reports', 'report_all_%s.html' % rtype)
    os.makedirs(os.path.dirname(out), exist_ok=True)
    catalog = load_json('catalog.json') or {}
    history = load_json('history_latest.json') or {}
    if not catalog:
        print('ERROR: no data/catalog.json'); sys.exit(1)

    rows = build_rows_all(history, catalog)
    ...
    body = render_overview(rows, today)
    if rtype == 'daily':
        body += render_daily(rows, today)
    else:
        body += render_weekly_monthly(rows, rtype, today)

    html = """<!DOCTYPE html><html lang="zh"><head><meta charset="utf-8">
<title>moligod {t}（全子弹版） · {today}</title>
<style>
...
<h1>moligod {t}（全子弹版） · {today}</h1>
<div class="sub">报告日期：{today}（{wd}）｜覆盖 {tot} 种子弹（3/4 级含完整统计，5 级实时行情）｜口径：低价期 {LOW} | 日常期 {DAY} | 出售到手=挂价×0.87（税13%）| SPIKE 不特殊处理 | 数据源 moligod.com</div>
{body}
<div class="note">数据口径：行情单位为哈夫币；出售实际到手 = 挂单价 × 0.87（13% 交易税）；覆盖全部 3/4/5 级子弹（低价期/日常期完整统计）；数据来自 moligod 公开行情，消息面来自公开网络信息（见各条来源），仅供游戏内交易参考。</div>
</div></body></html>""".format(
        t=t, today=today, wd=wd, tot=len(rows),
        LOW='%s~%s' % (LOW_START[5:], LOW_END[5:]),
        DAY='%s~%s' % (DAY_START[5:], DAY_END[5:]), body=body)
    with open(out, 'w', encoding='utf-8') as f:
        f.write(html)
    print('REPORT_ALL', rtype, 'rows=', len(rows), '->', out)
